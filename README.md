...
order optimization. We begin by analyzing a one-dimensional function, deriving its piecewise form, stationary points, and global minimizers. We then relate the analysis to gradient descent (GD) and stochastic gradient descent (SGD): step-size choices, behavior near non-smooth points, and typical failure modes.
